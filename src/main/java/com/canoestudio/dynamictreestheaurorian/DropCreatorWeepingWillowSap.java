...
public class DropCreatorWeepingWillowSap extends DropCreator {

    private final Item sapItem;
    private final int chance;

    public DropCreatorWeepingWillowSap(Item sapItem, int chance) {
        super(new ResourceLocation(DynamicTreesTA.MODID, "weepingwillowsap"));
    ...
    public List<ItemStack> getVoluntaryDrop(World world, Species species, BlockPos rootPos, Random random, List<ItemStack> dropList, int soilLife) {
        ...
        if (branch != null && branch.getRadius(trunk) >= 8) {
            if (0.33f > random.nextFloat()) {
                dropList.add(new ItemStack(sapItem, 1, 0));
            }
        }
        return dropList;
    }
